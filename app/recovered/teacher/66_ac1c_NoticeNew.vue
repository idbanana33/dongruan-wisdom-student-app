<!--
  Source: assets/www/js/66.js -> webpack module "ac1c"
...
<q-layout view="lHh lpr lFf">
      ...
      <q-form ref=""form"" class="full-width" @submit='saveRecord'>
          ...
          <q-list class="full-width">
            ...
            <q-item>
              <q-item-section>
                ...
                <q-uploader v-if="0 == isHarmonyOS" class="full-width" color="grey" flat :url='uploader' auto-upload :filter='checkFile' accept=".pdf, .pptx, .ppt, .xls, .xlsx, .doc, .docx, audio/*, image/*, video/*" @uploaded='afterUploaded' @rejected='onRejected' @removed='afterRemoved'>
                  ...
                  <template v-slot:list>
                    <q-list separator>
                      <q-item v-for="i in t.files" :key='i.name'>
                        <q-item-section>
                        ...
                        <q-item-section side>
                          <q-btn flat dense color="negative" icon="delete" size="sm" @click='t.removeFile(i)'>
                            <q-tooltip>
                              删除
                            </q-tooltip>
                          </q-btn>
                ...
                <q-uploader v-if="1 == isHarmonyOS" class="full-width" color="grey" flat :url='uploader' auto-upload :filter='checkFile' @uploaded='afterUploaded' @removed='afterRemoved'>
                  <template v-slot:header>
                    <div class="row no-wrap items-center q-pa-sm q-gutter-xs">
                      <div class="col">
                        附件上传(10M以内)
        ...
        <q-card class="full-width row q-mt-sm" flat>
          <q-list class="full-width">
            ...
            <q-item v-for="(t, i) in 'A' == record.frequency ? record.appointed_date : []" :key='t.begin_time'>
              <q-item-section avatar>
                <q-item-label>
                  {{ "时间段" + i + 1 }}
                </q-item-label>
              ...
              <q-item-section side>
                <q-item-label v-show="record.appointed_date.length > 1" class="text-negative cursor-pointer" @click='removeAppointedDate(i)'>
                  删除
                  <q-tooltip>
                    删除该时间段
                  </q-tooltip>
        ...
        <q-card class="full-width row q-mt-sm" flat>
          <q-list class="full-width">
            ...
            <q-separator />
            <q-item v-if="'C' != record.apply_type">
              <q-item-section avatar>
                <q-item-label class="neu-css-before-red-asterisk">
              ...
              <q-item-section />
              <q-item-section side>
                <q-toggle v-model="applyMaxToggle" size="lg" dense :false-value="0" :true-value="1" />
              </q-item-section>
            </q-item>
            <q-item v-if="1 == applyMaxToggle">
              <q-item-section>
                <q-input v-model="record.apply_max" v-show="applyMaxToggle == 1" outlined dense type="number" hide-bottom-space lazy-rules :rules='[function (t) { return !!t || 0 == applyMaxToggle || t > 0 || "请填写该项"; }]' />
              </q-item-section>
            </q-item>
            <q-separator v-if="'C' !== record.apply_type" />
            <q-item>
              <q-item-section avatar>
                <q-item-label class="neu-css-before-red-asterisk">
                  需要学生定位信息
              ...
              <q-item-section />
              <q-item-section side>
                <q-toggle v-model="need_location" size="lg" dense :false-value="0" :true-value="1" />
              </q-item-section>
            </q-item>
            <q-separator />
            <q-item>
              ...
              <q-item-section />
              <q-item-section side>
                <q-toggle v-model="allow_counsellor" size="lg" dense :false-value="0" :true-value="1" />
              </q-item-section>
            </q-item>
            <q-separator />
          </q-list>
            ...
            <q-item v-if="0 !== record.info_config.length">
              <q-item-section>
                <div class="row items-center">
                  <div class="col-12 col-md-2 col-sm-3 neu-css-after-colon" :class='{ "text-right": !$q.screen.lt.sm, "neu-css-before-red-asterisk": "C" == applyType }'>
                    信息收集项目
                  </div>
                  <div class="col-12 col-md-10 col-sm-9">
                    <div v-for="(t, i) in record.info_config" :key='i' class="bg-grey-2 q-pa-sm q-mt-sm">
                      <div>
                        <span class="neu-css-after-colon">
                          {{ "第" + i + 1 + "题" }}
                        </span>
                        <span class="text-primary cursor-pointer q-ml-sm" @click='editInfoConfig(i)'>
                          编辑
                        </span>
                        <span class="text-negative cursor-pointer q-ml-sm" @click='deleteInfoConfig(i)'>
                          删除
                        </span>
                        <span v-show="index + 1 != record.info_config.length" class="text-primary cursor-pointer q-ml-sm" @click='moveInfoConfig(i, 1)'>
                          下移
                        </span>
                        <span v-show="index != 0" class="text-primary cursor-pointer q-ml-sm" @click='moveInfoConfig(i, -1)'>
                          上移
                        </span>
                      </div>
  ...
  <q-footer class="bg-white text-primary" bordered>
    <div class="row justify-between">
      <q-btn style="width:40%;height:2.8rem" flat color="grey" label="取消" :to='"/teacher/apply/index?apply_type=" + e.applyType' />
