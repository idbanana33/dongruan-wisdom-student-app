<!--
  Source: assets/www/js/72.js -> webpack module "d79d"
...
<q-layout view="lHh lpr lFf">
  ...
  <q-dialog v-model="extendDialog" persistent>
    <q-card class="full-width" flat>
      <q-form @submit='extendJoblog'>
        <q-card-section class="text-h6">
          {{ "\n              " + 1 == record.status ? "确认延期" : "确认重新开启" + "\n            " }}
        </q-card-section>
        <q-card-section>
          <div class="row items-center">
            {{ "\n                " + 1 == record.status ? "延期" : "重新开启" + "截止日期至：\n                " }}
            <q-field style="width:100%" outlined dense stack-label hide-bottom-space lazy-rules :rules='[function (e) { return !!e || "请填写该项"; }, function (t) { return t >= e.record.end_date || "请填写原截止日期之后的日期"; }]' :value='endDate'>
    ...
    <q-card class="full-width" flat>
      ...
      <q-card-section>
        <div>
          ...
          {{ 1 === record.allow_delete ? "是" : "否" + "\n            " }}
          <div style="float:right;margin-top:-7px">
            <q-toggle v-if="record.is_author" v-model="record.allow_delete" class="q-pa-sm" size="lg" checked-icon="check" unchecked-icon="clear" dense :false-value="0" :true-value="1" @input='changeAllowDelete' />
          </div>
        </div>
      </q-card-section>
      <q-separator />
      <q-card-section>
        <div>
          ...
          {{ 1 === record.allow_look ? "否" : "是" + "\n            " }}
          <div style="float:right;margin-top:-7px">
            <q-toggle v-if="record.is_author" v-model="record.allow_look" class="q-pa-sm" size="lg" checked-icon="check" unchecked-icon="clear" dense :false-value="1" :true-value="0" @input='changeAllowLook' />
          </div>
        </div>
      </q-card-section>
      <q-separator />
      <q-card-section>
        <div>
          ...
          <div v-if="record.is_author && null !== menuInfo.edit_permit" style="float:right">
        ...
        <neu-view-assigned-teacher :value='record.task_target' />
      </q-card-section>
      <q-separator />
      <q-card-section>
        <div>
  ...
  <q-dialog v-model="editDialog" persistent :maximized='true'>
    <q-card class="full-width" flat>
      <q-form @submit='submitJoblogResult'>
        <q-card-section class="row items-center justify-between">
          <div class="text-h6">
            工作日志
          </div>
        ...
        <q-separator />
        <q-card-section>
          <div v-for="(e, a) in infoConfig" :key='a' class="bg-grey-2 q-pa-sm q-mt-sm">
            <neu-widget v-model="result.info_result[index]" :config='e' :view-mode='viewMode' />
          </div>
        </q-card-section>
        <q-card-section class="row q-gutter-sm justify-end">
          <q-btn v-close-popup outline color="grey" label="取消" />
          <q-btn v-show="!viewMode" unelevated color="grey" label="暂存" type="submit" @click='result.status = 0' />
          <q-btn v-show="!viewMode" unelevated color="primary" label="提交" type="submit" @click='result.status = 1' />
        </q-card-section>
      </q-form>
    </q-card>
  </q-dialog>
  ...
  <q-pull-to-refresh ref=""pageRefresh"" @refresh='refresh'>
    <q-page-container>
      <q-page class="bg-grey-3 q-py-sm">
        <q-card v-for="e in list" :key='e.id' class="bg-white q-mb-sm q-mx-sm" :style='0 == e.status ? "border-left:0.25rem solid #b5b3b3;" : "border-left:0.25rem solid #027be3;"' flat @click='goDetail(e.id, !e.allow_edit, e.teacher_no, e.allow_look)'>
          <q-item clickable>
            <q-item-section>
              <q-item-label>
  ...
  <q-drawer v-model="drawerRight" side="right" :width="200" :breakpoint="100" overlay bordered content-class="bg-white-3">
    <q-scroll-area class="fit">
      <q-list style="border-left:0.15rem solid #027be3" separator>
        ...
        <q-item v-if="1 == record.status && record.is_author" v-ripple clickable @click='openExtendDialog'>
          <q-item-section avatar>
        ...
        <q-item v-if="0 == record.status && record.is_author" v-ripple clickable @click='openExtendDialog'>
          <q-item-section avatar>
        ...
        <q-item v-if="record.is_author || 'A' == roleId" v-ripple clickable @click='deleteJoblog'>
          <q-item-section avatar>
        ...
        <q-item v-if="0 == record.allow_look || 'A' != roleId" v-ripple clickable @click='exportList'>
          <q-item-section avatar>
  ...
  <q-page-sticky v-if="aCreate" position="bottom-right" :offset='fabPos'>
    <q-btn v-touch-pan.prevent.mouse.prevent.mouse="moveFab" round color="primary" icon="edit" @click='goDetail(null, false)' />
  </q-page-sticky>
</q-layout>
</template>
