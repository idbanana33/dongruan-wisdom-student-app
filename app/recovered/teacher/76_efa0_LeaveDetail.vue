<!--
  Source: assets/www/js/76.js -> webpack module "efa0"
...
<q-layout view="lHh lpr lFf">
  <q-header class="bg-grey-3 text-black">
    <q-toolbar>
      ...
      <q-btn v-if="'A' == userinfo.role_id && 'A' == record.review_status && null === record.leave_multi_id && '已销假' !== record.report_status_name" class="q-mr-sm" flat round dense icon="clear" color="negative" @click='RefuseLeave' />
    </q-toolbar>
  </q-header>
  <q-page-container>
    <q-page class="bg-grey-3">
      ...
      <q-card v-if="0 == pageType && menuInfo" class="full-width row q-mt-sm" flat>
        <q-card-section class="full-width row justify-around" avatar>
          ...
          <div class="col-6 q-mb-md text-right text-weight-bold">
            <span class="text-primary cursor-pointer" @click='getStudentHistory(record.student_no, record.name)'>
              查看历史请假
            </span>
          </div>
          <div class="col-6">
      ...
      <q-card v-if="0 == pageType" class="full-width row q-mt-sm" flat>
        <q-card-section class="full-width row justify-around" avatar>
          ...
          <div v-if="1 === record.need_report" class="col-12 q-mt-sm">
            <span class="text-grey">
              销假状态：
            </span>
            <span>
              {{ record.report_status_name }}
            </span>
            <span v-if="'已销假' == record.report_status_name">
              {{ "(" + record.report_time + ")" }}
            </span>
          </div>
          <div class="col-12 q-mt-sm">
          ...
          <div v-if="record.leave_multi_id" class="col-12 q-mt-sm">
            <span class="text-grey">
              批量请假教师：
            </span>
            <span>
          ...
          <div class="col-12 q-mt-sm">
            ...
            <span class="text-primary cursor-pointer" @click='showCourseInfo(record.student_no, record.name, record.begin_day, record.end_day)'>
              查看
            </span>
          </div>
          <div v-if="record.location" class="col-12 q-mt-sm">
            <span class="text-grey">
              发起位置：
            </span>
            <span>
          ...
          <div v-show="record.attachment && record.attachment.length != 0" class="col-12 q-mt-sm">
            ...
            <div v-for="t in record.attachment" :key='t.link' class="q-mt-sm">
          ...
          <q-item>
            <div v-if="0 == showReview" class="q-px-sm q-pb-xs">
              <q-timeline>
                ...
                <q-timeline-entry v-for="(t, a) in record.review_list" :key='t.id' :subtitle='t.review_time' :color='"W" != record.review_status && record.review_list && a == record.review_list.length - 1 ? "A" == t.review_status ? "positive" : "red" : "grey"'>
                  {{ "\n                  " + t.teacher_name + " - " + t.review_status_name + "\n                  " }}
                  <p v-show="item.opinion != null" style="font-size:11px">
                    {{ "审批意见：" + t.opinion }}
                  </p>
                </q-timeline-entry>
                <q-timeline-entry v-if="'W' != r_review.r_status && nextTeacher.length > 0" subtitle="当前审核教师" color="warning">
                ...
                <q-timeline-entry v-if="'W' == r_review.r_status" subtitle="现在" color="warning" icon="check">
                ...
                <q-timeline-entry v-if="record.report_time" :subtitle='record.report_time' color="grey">
                  {{ "\n                  " + record.name + " - 销假成功\n                  " }}
                  <div class="row full-width">
                    <q-img v-for="t in record.report_attachment" :key='t.link' class="q-mx-sm q-my-sm" style="cursor:pointer;height:auto;width:40%" :src='serverUrl + t.link' :ratio="1" @click='showBig(t.link, t.srcname)' />
                  </div>
                </q-timeline-entry>
              </q-timeline>
            </div>
            <div v-if="1 == showReview" class="q-px-sm q-pb-xs">
              <q-timeline>
                ...
                <q-timeline-entry v-for="(t, a) in review_list" :key='t.id' :subtitle='t.review_time' :color='"W" != record.review_status && record.review_list && a == record.review_list.length - 1 ? "A" == t.review_status ? "positive" : "red" : "grey"'>
                  {{ "\n                  " + t.teacher_name + " - " + t.review_status_name + "\n                  " }}
                  <p v-show="item.opinion != null" style="font-size:11px">
                    {{ "审批意见：" + t.opinion }}
                  </p>
  ...
  <q-dialog v-model="courseInfoDialog">
    <q-card class="full-width" flat>
      <q-card-section>
        <div class="text-bold">
          {{ courseInfoTitle + " 的课表及活动" }}
        </div>
      </q-card-section>
      <hr v-if="0 == courseInfo.length" />
      <div v-if="0 == courseInfo.length" class="text-h6 text-bold q-mb-md row justify-center full-width">
        当日暂无课表及活动信息！
      </div>
      <q-markup-table v-if="0 != courseInfo.length" bordered flat>
        ...
        <tbody>
          <tr v-for="t in courseInfo" :key='t.date'>
            <td>
              {{ t.date }}
            </td>
            <td v-html='t.content' />
  ...
  <q-dialog v-model="writeOpinion">
    <q-card style="width:700px;max-width:80vw">
      ...
      <q-card-section class="q-pt-none">
        <q-input v-model="opition" dense autofocus />
      </q-card-section>
      <q-card-actions class="text-primary" align="right">
        <q-btn v-close-popup flat label="取消" />
        <q-btn v-close-popup flat label="确认" @click='forceOperation()' />
      </q-card-actions>
    </q-card>
  </q-dialog>
</q-layout>
