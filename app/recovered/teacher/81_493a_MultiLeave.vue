<!--
  Source: assets/www/js/81.js -> webpack module "493a"
...
<q-layout view="lHh lpr lFf">
  <q-header class="bg-grey-3 text-black">
    <q-toolbar>
      ...
      <q-btn v-if="'review' == tab" flat round dense icon="search" @click='open'>
  ...
  <q-page-container>
    <q-page class="bg-grey-3 q-pb-sm">
      <q-pull-to-refresh ref=""pageRefresh"" @refresh='refresh'>
        <q-card v-for="(t, a) in data" v-touch-hold.mouse.mouse="(e)=>handleHold(index)" :key='t.id' class="bg-white q-mb-sm q-mx-sm" :style='"驳回" == t.review_name ? "border-left:0.25rem solid #FF0000;" : "审批中" == t.review_name ? "border-left:0.25rem solid #20B2AA;" : "border-left:0.25rem solid #32CD32;"' flat @click='goDetail(t.id, t.leave_multi_id, a)'>
          <q-item>
            <q-item-section v-show="tab === 'review' && operateDisplay && item.review_status == 'W'" avatar>
              <q-item-label>
                <q-checkbox v-model="selected" :id='"leaveMultiItem" + a' :val='t' />
              </q-item-label>
            </q-item-section>
            <q-item-section>
              <q-item-label>
      ...
      <q-page-sticky position="bottom-right" :offset='fabPos'>
        <q-fab v-model="fab" v-touch-pan.prevent.mouse.prevent.mouse="moveFab" v-show="tab === 'review' && operateDisplay" label="操作" icon="keyboard_arrow_up" direction="up" color="primary" :disable='draggingFab'>
      ...
      <q-page-sticky v-if="'leave' == tab && menuInfo || {  }.edit_permit" position="bottom-right" :offset='fabPosAdd'>
        <q-btn v-touch-pan.prevent.mouse.prevent.mouse="moveFabAdd" round color="primary" icon="add" @click='onClick' />
      </q-page-sticky>
    </q-page>
  </q-page-container>
